...
export default async function AdminInvoicesPage() {
  ...
  const { data: invoices } = await supabase
    .from("invoices")
    .select("id, invoice_number, amount, payment_status, created_at, customers(profiles(full_name))")
    .order("created_at", { ascending: false })
    .limit(100);

  return (
    <div>
      <h1 className="text-2xl font-extrabold tracking-tight mb-6">كل الفواتير</h1>

      {!invoices || invoices.length === 0 ? (
        <EmptyState title="مفيش فواتير مُسجّلة لسه" />
      ) : (
